...
function draw(){
  ...
  if (mode === "tree") svgEl.setAttribute("viewBox", `0 0 900 ${Math.max(h, 400)}`);
  if (mode === "tree") setCount(`${list.length} of ${total} shown`);
  applyTransform();
}
...
function seedPositions(){
  ...
  for (const n of G.nodes){
    ...
    n.y = CY + Math.sin(ang) * r * 0.72;    // the viewport is wider than it is tall
    n.vx = n.vy = 0;
  }
}
...
function fitView(){
  ...
  const pad = 70;
  const s = Math.min(4, Math.max(0.25,
    Math.min(W / (x1 - x0 + pad * 2), H / (y1 - y0 + pad * 2))));
  const c = cam.graph;
  c.s = s;
  c.tx = W / 2 - s * (x0 + x1) / 2;
  c.ty = H / 2 - s * (y0 + y1) / 2;
...
canvas.addEventListener("wheel", e => {
  ...
  const c = cam[mode];
  c.s = Math.min(4, Math.max(0.25, c.s * k));
  applyTransform();
  if (mode === "graph"){
    userMoved = true;
...
// A node press must set the pan origin too, so the drag-threshold maths below
// has a start point even though the canvas handler was stopped.
gview.addEventListener("pointerdown", e => { startX = e.clientX; startY = e.clientY; }, true);
